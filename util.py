...
def init_db_conn() -> MockConnection:
    stage = os.environ['BACKEND_STAGE']

    with open('conf/data.{}.json'.format(stage)) as json_file:
        data = json.load(json_file)
        key = data['encryption_key'].encode()
        db_username = decrypt(key,data['pg_talk_user'],block_segments=True).decode()
        db_password = decrypt(key,data['pg_talk_password'],block_segments=True).decode()
        url = data['master_dsn_no_cred'].format(db_username, db_password)
        return create_engine(url, pool_size=256)

def decrypt(key: bytes, value: str, block_segments=False) -> bytes:
...
def encrypt(key: bytes, value: bytes, block_segments=False) -> bytes:
